...
countries_sorted = sorted(conf['countries'].values(), key = lambda k: 
        (k['countryclass'],  k['alias']))
...
ages_sorted = sorted(conf['ages'].values(), 
        key = lambda k: (k['ptype'],  k['agegroups']))
...
indextempl.stream(compyrseq = list(range(1952, 2022, 10)), 
        countries = countries_sorted, causes = causes_sorted, ages = ages_sorted, 
        causeclasses = conf['causeclasses']).dump(indexfname + '.html')
doctempl.stream(compyrseq = list(range(1952, 2022, 10)), causes = causes_sorted, 
        causeclasses = conf['causeclasses']).dump(docfname + '_norefhead.md')
